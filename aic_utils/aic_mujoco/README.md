...
The command trains only; policy execution and simulation evaluation remain
separate future capabilities. The committed W&B mode is online, so the user
must authenticate W&B before starting the run.
...
before the policy configuration. No command-line or environment-variable
override can silently change these contracts.
...
and training refuses to overwrite an existing run directory.
