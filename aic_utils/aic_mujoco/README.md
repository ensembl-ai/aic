...
This package provides four deliberately separate capabilities:
...
3. a privileged Cartesian teacher that generates synthetic demonstration
   trajectories; and
4. supervised training of a three-camera visuomotor manipulation policy.
...
Policy training uses the additional strict
[`configs/train.json`](configs/train.json) overlay. After the configured minimum
train and validation episodes are complete, start training with:

```bash
pixi run python aic_utils/aic_mujoco/scripts/train_policy.py
```

The training command does not launch simulation. The committed W&B mode is
online, so the user must authenticate W&B before starting the run.

After training, evaluate the explicit checkpoint on the complete held-out test
split:

```bash
pixi run python aic_utils/aic_mujoco/scripts/validate_policy.py
```

The resulting JSON report contains overall and per-joint action errors. Run the
same checkpoint as a closed-loop controller in one continuously randomized
MJWarp environment with:

```bash
pixi run python aic_utils/aic_mujoco/scripts/run_policy.py
```

Viser shows the robot and scene at `http://127.0.0.1:8080`. Each episode resets
after success or the configured time limit. Execution ends after
`evaluation.rollout_episodes`, or earlier on `Ctrl+C`. Both workflows use the
strict [`configs/evaluate.json`](configs/evaluate.json) overlay; neither accepts
command-line behavior overrides or searches for a "latest" checkpoint.
`evaluation.maximum_episode_steps` controls the learned policy timeout
independently of the teacher trajectory horizon in
`expert.maximum_episode_steps`. At 20 Hz, 500 policy steps represent 25 seconds
of simulated time.
...
#### Supervised visuomotor policy

The training baseline uses LeRobot's open-source ACT transformer as a
closed-loop manipulation policy, not as a separate pose estimator. One sample
contains the synchronized center/left/right RGB images and the robot's six
joint positions and velocities. The target is a chunk of future bounded
six-joint increments generated by the privileged Cartesian teacher:

```text
center RGB + left RGB + right RGB + qpos + qvel
                         ↓
              shared visual backbone
                         ↓
                ACT transformer
                         ↓
        16 future action_delta_q vectors
```

ACT learns temporally coherent action chunks while retaining the simple
low-level interface already accepted by the impedance controller. The
configured objective combines action L1 error with ACT's variational latent
regularization. Images use explicit ImageNet channel normalization; joint state
and actions use mean and standard deviation computed only from the training
split. Validation reports action mean absolute error both in normalized units
and joint radians.

Every collected episode currently has the same instruction. A language branch
would therefore receive no varying supervisory signal, so this MVP does not
claim to be a VLA. The policy boundary remains isolated so a language-conditioned
architecture can replace ACT when the dataset contains multiple instructions
and behaviors.

At inference, the policy replans at the 20 Hz camera rate. The first joint
increment from each predicted chunk is bounded using the same limits as the
teacher and converted into a new HOLD target. The existing 200 Hz impedance
controller then tracks that target through MJWarp physics:

```text
three RGB views + qpos + qvel
              ↓
             ACT
              ↓
bounded six-joint increment
              ↓
joint HOLD target → impedance control → MJWarp
```

Exact simulated SFP and goal poses are used only to decide whether an episode
succeeded and should reset. They are never provided to the learned policy.
...
│   ├── evaluation.py   held-out action metrics and report generation
│   ├── inference.py    closed-loop learned-policy execution and resets
│   ├── joints.py       named joint/actuator address mapping
│   ├── outputs.py      selected-world Viser geometry bridge
│   ├── policy.py       configured LeRobot policy construction
...
│   ├── training.py     optimization, validation, checkpoints, and W&B
│   ├── training_data.py episode decoding and action-chunk sampling
│   └── utils/          reusable array, image, video, and numerical operations
...
│   ├── train.json   policy/optimizer/observability overlay
│   └── evaluate.json checkpoint/validation/inference overlay
├── mjcf/
│   ├── aic_robot.xml
│   ├── aic_world.xml
│   └── scene_warp.xml
...
or reward layers. ACT learns the same `JointDeltaAction` produced by the
teacher, so later policy execution will not require a different physics or
impedance-control interface.

#### Base and execution overlays

Each workflow loads only its complementary configuration layers. `base.json`
...
collector deep-merges `base.json + collect.json`; training deep-merges
`base.json + collect.json + train.json` so the dataset contract is validated
before the policy configuration. Validation and inference add `evaluate.json`
as the final layer. No command-line or environment-variable override can
silently change these contracts.
...
A teacher attempt enters none of these splits unless both Cartesian tolerances
hold for `expert.success_consecutive_steps`. A timed-out attempt is deleted and
resampled. Every published episode therefore represents a successful teacher
trajectory; increasing `expert.maximum_episode_steps` changes how long the
teacher may try, not the definition of success.

Raw datasets, checkpoints, W&B binaries, and other heavyweight run artifacts
are ignored by Git. Compact evidence under each run's `metrics/` directory is
explicitly trackable.

#### Training lifecycle and observability

Training snapshots only atomically complete successful episodes at startup, so
it can run while collection continues without observing partial files. The
loader decodes one episode per worker, samples aligned action chunks, and
releases that episode before decoding another; it does not expand the complete
video dataset into memory.

W&B records train loss components, gradient norm, learning rates, data/update
timing, sample throughput, and validation action errors. Run summaries retain
the dataset snapshot size, model parameter count, and latest checkpoint.
Checkpoints contain LeRobot-compatible policy weights and train-split
normalization statistics. A checkpoint is published by atomic directory rename,
and training refuses to overwrite an existing run directory. The same scalar
events are also appended to `metrics/training/history.jsonl`, so comparison and
report generation do not depend on W&B availability.

Offline evaluation decodes every episode in the configured held-out split and
reports normalized action error, action error in radians, and per-joint error.
It loads one explicit checkpoint and checks only values that must match the
saved tensor interface: ordered joints, policy input image shape, and policy
architecture/preprocessing. Episode horizons, success criteria, controller
settings, physics, visualization, and domain randomization remain adjustable
evaluation behavior and do not invalidate the checkpoint. Closed-loop
inference uses the same checkpoint loader and normalization statistics, keeping
offline metrics and simulation execution on one model interface.

Closed-loop inference records one world-frame SFP and goal sample at every
policy step. The incremental path contains position/orientation error,
step distance, cumulative workspace path length, and progress toward the goal.
Every success, timeout, or interrupted episode also records final/closest error,
path length, displacement, straightness, and duration. An aggregate summary
tracks success rate and mean/median episode metrics. These JSON/JSONL artifacts
are stored under `metrics/rollouts/<checkpoint>/<session>/` and remain valid
even when execution is stopped with `Ctrl+C`.
